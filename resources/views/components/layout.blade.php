...
<div class="flex flex-row items-center gap-4 ml-auto">
    ...
    <div class="relative flex items-center" id="more-menu-container">
        ...
        <div id="more-dropdown" class="hidden absolute right-0 top-12 mt-2 w-64 bg-white rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] border border-slate-200 z-[99999] text-xs pb-2">
            ...
            <!-- Section 3: About Us -->
            <div class="px-3 pt-1">
                <a href="{{ route('about') }}" class="flex justify-between items-center px-3 py-2 text-xs text-slate-600 font-bold hover:text-slate-800 hover:bg-slate-50 rounded-xl transition-colors group">
                    {{ __('About Us') }}
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const refreshWatchlistUI = () => {
            ...
            // Handle Empty State
            if (watchlist.length === 0 && sidebarContainer) {
                sidebarContainer.innerHTML = '<div class="text-[10px] font-bold text-slate-400 px-2 py-2 uppercase tracking-widest">No starred matches.</div>';
            }
            ...
            [...watchlist].reverse().forEach(savedItem => {
                ...
                // Inject into Sidebar with truncation, hover-title, and REMOVE button
                if (sidebarContainer) {
                    const sidebarHtml = `
                        <div class="group flex items-center justify-between hover:bg-slate-50 p-1.5 rounded-xl transition-colors" id="sidebar-item-${savedItem.id}">
                            <a href="/predictions/${savedItem.id}" class="flex items-center gap-2 text-xs font-bold text-slate-600 hover:text-slate-900 flex-1 truncate">
                            ...
                            <button type="button" 
                                    onclick="window.removeSidebarWatchlist('${savedItem.id}')"
                                    class="hidden group-hover:flex items-center justify-center w-6 h-6 rounded-md hover:bg-red-100 text-slate-300 hover:text-red-600 transition-colors shrink-0 ml-1" 
        ...
        // Attach function globally so the inline onclick button can reach it
        window.removeSidebarWatchlist = (id) => {
            const existsIndex = watchlist.findIndex(item => String(item.id) === String(id));
            if (existsIndex > -1) {
                watchlist.splice(existsIndex, 1);
                localStorage.setItem(storageKey, JSON.stringify(watchlist));
                refreshWatchlistUI(); // Automatically resets the stars on the table too!
            }
        };
